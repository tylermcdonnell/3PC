...
public class Launcher {
	...
	/**
	 * Instruct process to vote NO for the next transaction.
	 */
	private static void rejectNextChange(Integer id) {
		processes.get(id).nextDecision(false);
	}
	...
	private static void killLeader() {
		for (int i = 0; i < processes.size(); i++) {
			if (processes.get(i).isCoordinator(0)) {
				kill(i);
			}
		}
	}
	...
	/**
	 * Creates the specified number of "processes" (threads). For each thread
	 * created, add its handle to a list.
	 * 
	 * @param numProcesses,
	 *            the specified number of "processes" to create.
	 */
